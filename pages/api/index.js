        ...
        try {
                ...
                .aggregate(
                        [{ $match: {upc_code: req.query.upc_code}},
                         //{ $project: { product_name:1, bin:1, pcs_qty:1, item_code:1}},
                         //{$group: {_id: {bin:'$bin'}, Total:{$push:'$pcs_qty'}}}
                        ]
                        //[{$group: {bin:'$bin', Total:{$sum: '$pcs_qty'}}}]
                )
                .toArray();
...
app.get("/upcsearchsum", async (req, res) => {
        try {
                if (req.query) {
                let results;
                results = await client
